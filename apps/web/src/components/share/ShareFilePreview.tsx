...
export function ShareFilePreview({
  ...
  }, [isFont, getPreviewUrl, file.name]);
  ...
  useEffect(() => {
    if (isWord && !officeUseOnlineViewer) {
      loadDocxPreview();
    }
  }, [isWord, loadDocxPreview, officeUseOnlineViewer]);
  ...
  }, [isCsv, loadCsvPreview]);

  useEffect(() => {
    if (isZip) {
      loadZipPreview();
    }
  }, [isZip, loadZipPreview]);

  useEffect(() => {
    if (isFont) {
      loadFontPreview();
    }
  }, [isFont, loadFontPreview]);
            ...
            <div className="w-full h-full flex flex-col relative">
              {officeUseOnlineViewer && getPreviewUrl() && !officeOnlineError ? (
                ...
                <>
                  ...
                  ) : pptUseOnlineViewer && getPreviewUrl() && !pptOnlineError ? (
